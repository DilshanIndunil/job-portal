...
export const getAppliedJobs = async (req ,res) => {
    try {
        ...
        if(!application){return res.status(404).json({message:'No job Applicatons', success:false})};
...
export const getApplicants = async (req , res) => {
    try {
        ...
        if(!job) return req.status(404).json({message:'No Applicants Applied Yet', success:false});
